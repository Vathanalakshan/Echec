...
public class Echiquier {
	 		...
	 		if (disty==0){//deplacement vertical
	 			...
	 			for(int i=1;i<Math.abs(distx);i++){		
	 			 ...
	 			 if( echi[cntrx][cntry]!=null){
		 				System.out.print(echi[cntrx][cntry]);
	 				 return false;
	 			 }
	 			 cntrx=cntrx+dx;
	 	...
	 	public void deplacer(int xi,int yi,int xf,int yf){//i=initial f=final x,y position
	 		...
	 		if (getEchi()[xi][yi].getPiece().moveValable(xi,yi,xf,yf)==true && cheminLibre(xi,yi,xf,yf)==true){//verfie le deplacement est valable pour la piece
